...
async function getOmdbData() {
    ...
    const getOmdbInfo = async (imdbId) => {
        const apiKeys = ["8c967f70", "dd37e5a4", "3fdb9c5a", "b81150c9", "2981ebb6", "f17eacb0"];
    ...
    omdbDataPromise = (async function () {
        try {
            const result = await getOmdbInfo(imdbId);
            console.log("OMDB: ", result);
            if (!result) {
                throw new Error("No data found for the provided IMDb ID.");
            }

            console.log(`Rotten Tomatoes Rating: ${result.Ratings[1]?.Value || "N/A"}`);
            console.log(`Metacritic Rating: ${result.Ratings[2]?.Value || "N/A"}`);
            console.log(`Metascore: ${result.Metascore}`);

            return {
                source: "OMDB",
                ratingRottenTomatoes: result.Ratings[1]?.Value || 0,
                ratingMetacritic: result.Ratings[2]?.Value || 0,
                ratingMetascore: result.Metascore,
            };
        } catch (error) {
    ...
    return omdbDataPromise;
}

async function addOmdbRatingBadge() {
    const authorsMode = await GM_getValue("authorsMode", false);
    if (!authorsMode) return;

    const newRatingBadge = createRatingBadgeTemplate("OMDB");
    ...
    const ratingData = await getOmdbData();

    updateRatingTemplate(newRatingBadge, ratingData);
}
